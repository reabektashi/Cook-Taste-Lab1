...
// GET /api/admin/overview?period=7days|30days|365days
...
// GET /api/admin/reports?days=7|14|30
app.get("/api/admin/reports", auth, requireRole("admin"), async (req, res) => {
  try {
    const days = Math.max(1, Math.min(365, Number(req.query.days || 7)));

    // Top recipes by favorites (DB truth)
    // favorites table assumed: favorites(user_id, recipe_id, created_at?)
